...
, CONSTRAINT MEMBERSCORE_ENG_CK CHECK(KOR BETWEEN 0 AND 100)
, CONSTRAINT MEMBERSCORE_MAT_CK CHECK(KOR BETWEEN 0 AND 100)
...
--○ 전체 리스트 조회 쿼리문 구성(조인) → 개선 → LEFT JOIN 
SELECT M.SID, M.NAME, M.TEL
    , S.KOR, S.ENG, S.MAT
FROM TBL_MEMBER M, TBL_MEMBERSCORE S
WHERE M.SID = S.SID(+);
...
--○ 전체 리스트 조회 쿼리문 구성(조인) → 개선 → LEFT JOIN → 개선 → NVL
SELECT M.SID, M.NAME, M.TEL
    , NVL(S.KOR,-1) AS KOR
    , NVL(S.ENG,-1) AS ENG
    , NVL(S.MAT,-1) AS MAT
...
--==>>
/*
1	이윤태	010-1111-1111	91	81	71	243	81	1
2	오이삭	010-2222-2222	80	70	60	210	70	2
3	신시은	010-3333-3333	-1	-1	-1	-3	-1	3
4	최문정	010-4444-4444	-1	-1	-1	-3	-1	3
5	이호석	010-5555-5555	-1	-1	-1	-3	-1	3
*/
